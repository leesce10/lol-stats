...
| 6 | **플랫폼/배포** | Electron 자체앱, 트레이, GitHub Actions 자동 .exe/.dmg, 자동 업데이트 | ✅ |
| 7 | **설정 창** | 트레이→설정. 기능(브리핑/아이템/복귀/교전)별 오버레이·음성 개별 토글 + 말투(반말 기본/존댓말) + 전체 볼륨/음소거, 파일 영속 | ✅ |
| 8 | **음성 말투** | 기본 친근한 반말, 존댓말 선택 가능. 모든 TTS가 playTts에서 일괄 변환(로컬·서버 문장 공통) | ✅ |
...
- [x] **음성 설정** — on/off·볼륨·카테고리(브리핑/아이템/복귀/교전) 토글 (설정 창, 2026-06-30)
...
- [x] **설정 UI(작은 창)** — 기능별 오버레이·음성 토글 + 볼륨/음소거 파일 저장(2026-06-30)
- [x] **자동 업데이트** — electron-updater로 .exe 자동 갱신(재다운 불필요)
