...
// 키(예: "Darius","Zed") → 프로파일. 같은 id 중복 시 마지막이 우선.
export const PROFILE_BY_KEY: Record<string, ChampionProfile> = {};
for (const p of ALL) if (p && p.id) PROFILE_BY_KEY[p.id] = p;

export function getProfileByKey(key?: string | null): ChampionProfile | null {
  if (!key) return null;
